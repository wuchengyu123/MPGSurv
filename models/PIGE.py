...
# 		return self.layer(embeds)  # 行求softmax后取log

class UnsupervisedLoss(object):
	"""docstring for UnsupervisedLoss"""
	def __init__(self, adj_lists, train_nodes,POS_WALK,NEG_WALK, device):
		super(UnsupervisedLoss, self).__init__()
		self.Q = 15
		self.N_WALKS = 5
		self.WALK_LEN = POS_WALK
		self.N_WALK_LEN = NEG_WALK
		self.MARGIN = 3
	...
	def get_loss_sage(self, embeddings, nodes):
		...
		for node in self.node_positive_pairs:
			...
			#nodes_score.append(torch.mean(-pos_score).view(1,-1))
...
class GraphSage(nn.Module):
	...
	def forward(self, nodes_batch):
		...
		"""
		#print('nodes_batch',nodes_batch)
		lower_layer_nodes = list(nodes_batch)
		nodes_batch_layers = [(lower_layer_nodes,)]
		# self.dc.logger.info('get_unique_neighs.')
		for i in range(self.num_layers):
			lower_samp_neighs, lower_layer_nodes_dict, lower_layer_nodes= self._get_unique_neighs_list(lower_layer_nodes) # samp_neighs, unique_nodes, _unique_nodes_list
			nodes_batch_layers.insert(0, (lower_layer_nodes, lower_samp_neighs, lower_layer_nodes_dict)) # (unique_nodes_List,samp_neighs,unique_nodes)
			#
		...
		for index in range(1, self.num_layers+1):
			nb = nodes_batch_layers[index][0]  # unique_nodes_list
	...
	def _get_unique_neighs_list(self, nodes, num_sample=10):
		...
		#print('samp_neighs',samp_neighs) # {748, 1941, 1926}
		_unique_nodes_list = list(set.union(*samp_neighs))
		#print('unique_nodes_list',_unique_nodes_list)
...
if __name__ == '__main__':
	...
	#graphSage = GraphSage(2, feature_data.size(1), 128, feature_data, getattr(datacenter, 'cora' + '_adj_lists'), device=,gcn='store_true', agg_func='MEAN')
	#
